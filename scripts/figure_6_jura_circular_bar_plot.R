...
for (mm in seq(1:12))
{
  ...
  for (row in (1:nrow(df)))
  {
    ...
    #dmax = propa.detection_distance(L_bkg=unlist(rep(10,24)), L0=unlist(df[row,select_bandwidth[,2]]), f=fn, r0= 1, delta_r=1, t=unlist(df[row,'temperature']), rh=10, pa=unlist(df[row,'ps0']), a0=unlist(df[row,'a0']))
