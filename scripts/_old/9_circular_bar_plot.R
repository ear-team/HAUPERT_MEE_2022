...
# simulation with L0 = 80 dB for each frequency bin
L0 = 80 # Initial Sound pressure level
L0_per_bin = propa.dBSPL_per_bin(L=L0, f=seq(1,20,1))$db[1]
L0 = L0_per_bin
...
## ===================================================================================================================
#              loop to construct the detection distance of the 2 devices in 2 env when a sound at 80dB is produced
# ===================================================================================================================
...
for (mm in seq(1:12))
{
  ...
  p = ggplot(data, aes(x=as.factor(id), y=value, fill=group)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
    ...
    geom_bar(aes(x=as.factor(id), y=rep(20,length(id)), fill=light), stat="identity", alpha=0.4, width = 1) +
    
    # Add a val=300/250/200/150/100/50 lines. I do it at the beginning to make sur barplots are OVER it.
    geom_segment(data=grid_data, aes(x = end, y = 300, xend = start, yend = 300), colour = "grey", alpha=0.75, size=0.3 , inherit.aes = FALSE ) +
    geom_segment(data=grid_data, aes(x = end, y = 250, xend = start, yend = 250), colour = "grey", alpha=0.75, size=0.3 , inherit.aes = FALSE ) +
    ...
    # Add text showing the value of each 300/250/200/150/100/50 lines
    ylim(-100,375) +
    theme_minimal() +
    theme(
    ...
    annotate("text", x = rep(max(data$id),6), y = c(50, 100, 150, 200, 250, 300), label = c("50m", "100m", "150m", "200m","250m","300m") , color="black", size=2.5 , angle=0, fontface="bold", hjust=1) +
    # Add month legend
    annotate("text", x = c(1), y = c(290), label = c(month[mm]) , color="black", size=6 , angle=0, fontface="bold", hjust=-1) +
...
ggsave(file=paste(SITE,'_circular_plot.pdf',sep=''), mp, width = 18, height = 24, units = "cm")
